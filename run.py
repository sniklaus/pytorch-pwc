...
def estimate(tensorInputFirst, tensorInputSecond):
	...
	if True:
		...
		intWidth = int(math.floor(math.ceil(intWidth / 64.0) * 64.0))
		intHeight = int(math.floor(math.ceil(intHeight / 64.0) * 64.0))
		...
		tensorOutput.resize_(2, intHeight, intWidth).copy_(20.0 * torch.nn.functional.upsample(input=moduleNetwork(tensorPreprocessedFirst, tensorPreprocessedSecond), size=(intHeight, intWidth), mode='bilinear', align_corners=False)[0, :, :, :])
	# end
